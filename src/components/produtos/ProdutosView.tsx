import { Box, Database, PackageX, Shield, Zap } from 'lucide-react';
...
export const ProdutosView: React.FC = () => {
  ...
  useEffect(() => {
    ...
    window.addEventListener('virtude_data_synced', carregar);
    return () => {
      window.removeEventListener('storage', carregar);
      window.removeEventListener('virtude_data_synced', carregar);
    };
  }, []);
    ...
    <div className="space-y-6 pb-8">
      ...
      {produtos.length === 0 ? (
        <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-12 text-center space-y-3">
          ...
          <h3 className="text-sm font-bold text-slate-300">Nenhum Modelo de Produto Cadastrado</h3>
          <p className="text-xs text-slate-500 max-w-md mx-auto">
            Os modelos de Big Bags e especificações de engenharia serão indexados automaticamente a partir das novas planilhas e OPs programadas.
          </p>
        </div>
      ) : (
